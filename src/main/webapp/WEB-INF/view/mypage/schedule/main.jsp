...
$(function() {
	calendar=$('#calendar').fullCalendar({
		...
		events: function(start, end, timezone, callback){
			...
			var url= '<%=cp%>/mypage/schedule/events';
...
function insertForm(start, end, resource) {
	...
	if(resource) {
		...
		var url="<%=cp%>/mypage/schedule/readResourceList";
	...
	$('#resourceModal').load("<%=cp%>/mypage/schedule/inputForm", function() {
...
function changeGroup(groupNum, resourceNum) {
	...
	var url="<%=cp%>/mypage/schedule/listResourceList";
...
function insertOk() {
	...
	var url="<%=cp%>/mypage/schedule/schedulerInsert";
...
function updateOk() {
	...
	var query=$("form[name=schedulerForm]").serialize();
	var url="<%=cp%>/mypage/schedule/schedulerUpdate";
...
function deleteOk(num) {
	if(confirm("삭제 하시겠습니까 ?")) {
		$.post("<%=cp%>/mypage/schedule/schedulerDelete", {num:num}, function(data){
			var state=data.state;
			if(state=="false") {
